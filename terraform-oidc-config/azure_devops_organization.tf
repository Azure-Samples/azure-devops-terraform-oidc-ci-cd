locals {
  azure_devops_organization_id = jsondecode(data.http.azure_devops_organization.response_body).value[0].id
  azure_devops_base_64_pat = base64encode(":${var.azure_devops_token}")
}

data "http" "azure_devops_organization" {
  url = "https://dev.azure.com/${var.azure_devops_organisation_target}/_apis/projectCollections"
